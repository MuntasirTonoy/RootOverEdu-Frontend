  ...
  return (
    <div className="group bg-card border border-border rounded-3xl p-6 hover:shadow-xl hover:border-primary/30 transition-all duration-300 flex flex-col h-full relative overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col h-full">
          ...
          <h3 className="text-xl font-bold text-card-foreground mb-2 group-hover:text-primary transition-colors">{subject.title}</h3>
          <p className="text-sm text-muted-foreground mb-6 line-clamp-2">{subject.description}</p>
          
              
              <div className="mt-6 pt-4 border-t border-border flex gap-3">
                  <Link href={`/learn/${courseId}/${subject.id}`} className="flex-1 py-3 rounded-xl bg-surface hover:bg-primary-hover hover:text-white text-foreground font-bold text-center text-sm hover:-translate-y-1 transition-all flex items-center justify-center gap-2 group/btn">
                      Start Learning
                      <span className="material-icons text-sm group-hover/btn:translate-x-1 transition-transform"><MoveRight /></span>
                  </Link>
              </div>
      </div>
    </div>
