...
export default function LessonSidebar({ chapters, currentChapterId, currentPartId, onSelect, onClose }) {
  ...
  return (
    <div className="h-full bg-background border-r border-border flex flex-col transition-colors duration-300">
      <div className="p-6 border-b border-border flex justify-between items-center">
         ...
         <button onClick={onClose} className="lg:hidden p-2 hover:bg-surface rounded-lg text-foreground">✕</button>
      </div>

      <div className="flex-1 overflow-y-auto p-4 space-y-4">
         {chapters.map(chapter => (
            <div key={chapter.id} className="rounded-xl overflow-hidden border border-border">
               <button 
               ...
               >
                  ...
                  <span className={`transform transition-transform ${openChapters.includes(chapter.id) ? 'rotate-180' : ''}`}>
                     ▼
                  </span>
               </button>
               ...
               {openChapters.includes(chapter.id) && (
                  <div className="bg-card">
                     {chapter.parts.map(part => {
                        const isActive = part.id === currentPartId && chapter.id === currentChapterId;
                        return (
                           <button
                              key={part.id}
                           ...
                           >
                              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-[10px]
